...
const PRODUCT_DISPLAY_NAME = 'Lexify Premium Aylık'
const PRODUCT_USER_FACING_TITLE = 'Lexify Premium'
const PRODUCT_DURATION = 'P1M'

const APP_STORE_APP_NAME = 'Lexify iOS'
const APP_STORE_BUNDLE_ID = 'com.lexify.app'
const PLAY_STORE_APP_NAME = 'Lexify Android'
const PLAY_STORE_PACKAGE_NAME = 'com.lexify.app'

const ENTITLEMENT_IDENTIFIER = 'premium'
const ENTITLEMENT_DISPLAY_NAME = 'Premium Erişim'
...
async function seedRevenueCat() {
  ...
  if (priceError) {
    if (
      ...
      console.warn('Price error (non-fatal):', JSON.stringify(priceError))
    }
  } else {
  ...
  if (attachPackageError) {
    if (
      ...
      console.warn('Package attach warning:', JSON.stringify(attachPackageError))
    }
  } else {
  ...
  console.log('Public API Key (Test Store):', testStoreApiKeys?.items?.map((i) => i.key).join(', ') ?? 'N/A')
  console.log('Public API Key (App Store):', appStoreApiKeys?.items?.map((i) => i.key).join(', ') ?? 'N/A')
  console.log('Public API Key (Play Store):', playStoreApiKeys?.items?.map((i) => i.key).join(', ') ?? 'N/A')
